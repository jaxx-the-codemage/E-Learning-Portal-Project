				...
				<div class="upd">
					...
					if(isset($_POST['showCourse'])){
						...
						if(mysqli_num_rows($res)>0){
							...
							echo "<h3>**PLEASE SELECT THE FILE AGAIN.** <br> **EVEN IF YOU DON'T WANT TO CHANGE IT, SELECT THE SAME PREVIOUS FILE**</h3>";
							while($rec = mysqli_fetch_array($res)){
								echo "<form action='" . $_SERVER['PHP_SELF'] . "' method='POST' enctype='multipart/form-data'>";
								$fpath = $rec['fpath'];
								echo "<tr>
									<td><input type='text' value='$rec[mName]' name=txtmName></td>
									<td><input type=file name=fpath><br>
									<small>Current file: $rec[fpath] </small>
									</td>
									<td><input type='hidden' name='mid' value='{$rec['mid']}'>

									<button type=submit value='$rec[cid]' name=updateMaterial onclick=\"return confirm('Are you sure that you want to update the data of the material?')\">Update</button></td>
									 </tr>";
								echo "</form>";
							}
							echo "</table>";
...
if(isset($_POST['updateMaterial'])){
	...
	$cid = htmlspecialchars($_POST['updateMaterial']);

	$con = connect();
	$sql = "update materials set mName='$mname', fpath='$target' where cid='$cid' and mid='$mid'";
	$res = mysqli_query($con,$sql);

	if($res){
		echo"<script>alert('Updated Successfully.'); window.location.href='updateMaterial.php'</script>";
	}
	else{
