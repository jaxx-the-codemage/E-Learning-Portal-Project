		...
		<div class="right">
			...
			<div class="content">
				...
				<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
					<table>
						<tr>
							<td><label>Choose Course to Delete: </label></td>
							<td>
								<select name="cor"> 
									<?php
						...
						<tr>
							<td colspan="2"><center><input type="submit" onclick="return confirm('Are you sure to delete?')" name="deleteCourse"></center></td>
						</tr>
					</table>
				</form>
			</div>
...
if(isset($_POST['deleteCourse'])){
	$cName = htmlspecialchars($_POST['cor']);

	$con = connect();

	$sql = "delete from courses where cid='$cName'";
	$res = mysqli_query($con,$sql);

	if($res){
		echo"<script>alert('Data deleted'); window.location.href='deleteCourse.php'</script>";
	}
	else{
